...
This project uses DDEV for local development. All PHP, Composer, Artisan, Node, and database commands must be run through DDEV (e.g., `ddev exec`, `ddev composer`, `ddev npm`, `ddev artisan`, `ddev mysql`). Use DDEV's built-in commands where available; fall back to `ddev exec` for everything else.
